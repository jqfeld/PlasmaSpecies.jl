...
@testset "PlasmaReaction" begin
    ...
    @testset "apply_tree: rate numeric type is preserved" begin
        # N2[A,v=1] is a leaf (substrate, no branching); N2[B] has two leaves (product, branching_factor = 0.5)
        # → 2 expanded reactions each with rate 6.0f0 * 0.5 = 3.0f0, sum = 6.0f0
        tree = SpeciesTree([
            "N2[A,v=1]",
            "N2[B]",
            "N2[B,v=1]",
        ...
        pr = PlasmaReaction(6.0f0, ReactionFormula("N2[A,v=1] + e --> N2[B] + e"))
        expanded = apply_tree(tree, pr)

        @test all(r.rate isa Float32 for r in expanded)
        @test sum(r.rate for r in expanded) ≈ 6.0f0
    end
end
